...
export const WelcomeGate = () => {
    ...
    useEffect(() => {
        const accepted = localStorage.getItem("viper-welcomed");
        if (accepted === "true") {
            setStage("done");
        } else {
            // Small delay before showing gate for cinematic effect
            const t = setTimeout(() => setStage("gate"), 250);
            return () => clearTimeout(t);
        }
    }, []);

    const onAccept = () => {
        localStorage.setItem("viper-welcomed", "true");
        setStage("transition");
...
export const CookieBanner = () => {
    ...
    useEffect(() => {
        const checkAndShow = () => {
            const cookieChoice = localStorage.getItem("viper-cookies");
            const welcome = localStorage.getItem("viper-welcomed");
            setWelcomed(welcome === "true");
            if (!cookieChoice && welcome === "true") setVisible(true);
        };
        checkAndShow();
        // Listen for welcome-gate completion
        const t = setInterval(checkAndShow, 600);
        return () => clearInterval(t);
    }, []);

    const accept = () => {
        localStorage.setItem("viper-cookies", "accepted");
        setVisible(false);
    };
    const reject = () => {
        localStorage.setItem("viper-cookies", "rejected");
        setVisible(false);
    };
